    ...
    try {
      ...
      if (data) {
        const nameMap: Record<Language, string> = {
          en: data.site_name_en, fr: data.site_name_fr,
          ar: data.site_name_ar, ama: data.site_name_ama,
        };
        setSiteSettings({
    ...
    <>
      <style>{`
        ...
        .nav-logo-sub {
          ...
          color: rgba(201,168,76,0.55); line-height: 1;
        }
        ...
        .nav-link.active { color: var(--nav-gold); }
        ...
        .nav-cart:hover {
          background: rgba(201,168,76,0.16); border-color: rgba(201,168,76,0.45); color: var(--nav-gold);
        }
        .nav-cart-badge {
          position: absolute; top: -6px; right: -6px;
          min-width: 18px; height: 18px; padding: 0 4px;
          background: var(--nav-gold); color: var(--nav-deep);
          font-family: var(--nav-font); font-size: 0.62rem; font-weight: 800;
          display: flex; align-items: center; justify-content: center;
          border-radius: 2px; border: 1.5px solid rgba(10,31,14,0.8);
        }
        .nav-cart-ring {
          position: absolute; inset: -4px;
          border: 1px solid rgba(201,168,76,0.3);
          animation: cartPulse 2s ease-in-out infinite; pointer-events: none;
        }
        @keyframes cartPulse {
        ...
        .nav-lang-btn {
          ...
          color: rgba(255,255,255,0.65); font-family: var(--nav-font);
          font-size: 0.75rem; font-weight: 700; letter-spacing: 0.1em; text-transform: uppercase;
          cursor: pointer; transition: all 0.2s ease;
        }
        .nav-lang-btn:hover {
          border-color: rgba(201,168,76,0.45); color: var(--nav-gold); background: rgba(201,168,76,0.06);
        }
        .nav-lang-btn .chevron { transition: transform 0.25s ease; color: rgba(201,168,76,0.6); }
        .nav-lang-btn.open .chevron { transform: rotate(180deg); }

        .nav-lang-dropdown {
          position: absolute; top: calc(100% + 8px); right: 0; width: 176px;
          background: rgba(10,31,14,0.98); border: 1px solid rgba(201,168,76,0.2);
        ...
        .nav-lang-option:hover { background: rgba(201,168,76,0.06); color: rgba(255,255,255,0.9); }
        .nav-lang-option.active { color: var(--nav-gold); background: rgba(201,168,76,0.08); }
        .nav-lang-option-code {
          font-size: 0.65rem; font-weight: 800; letter-spacing: 0.15em;
          color: inherit; width: 24px; flex-shrink: 0;
        }
        .nav-lang-option-dot {
          width: 4px; height: 4px; border-radius: 50%;
          background: var(--nav-gold); margin-left: auto; flex-shrink: 0;
        }
        ...
        .nav-hamburger:hover {
          background: rgba(201,168,76,0.12); border-color: rgba(201,168,76,0.35); color: var(--nav-gold);
        }

        /* ── MOBILE DRAWER ── */
        .nav-drawer {
          background: rgba(10,31,14,0.99); backdrop-filter: blur(20px);
          border-top: 1px solid rgba(201,168,76,0.12);
          animation: drawerIn 0.3s ease; overflow: hidden;
        }
        @keyframes drawerIn {
        ...
        .nav-drawer-link-dot {
          ...
          background: rgba(201,168,76,0.3); flex-shrink: 0; transition: background 0.2s;
        }
        .nav-drawer-link.active .nav-drawer-link-dot,
        .nav-drawer-link:hover .nav-drawer-link-dot { background: var(--nav-gold); }

        .nav-drawer-lang { margin-top: 24px; padding-top: 20px; border-top: 1px solid rgba(201,168,76,0.1); }
        .nav-drawer-lang-title {
          font-family: var(--nav-font); font-size: 0.62rem; font-weight: 600;
        ...
        .nav-drawer-lang-btn {
          padding: 8px 16px; font-family: var(--nav-font);
          font-size: 0.78rem; font-weight: 700;
          border: 1px solid rgba(201,168,76,0.18); background: rgba(201,168,76,0.05);
          color: rgba(255,255,255,0.5); cursor: pointer; transition: all 0.2s;
        }
        .nav-drawer-lang-btn:hover {
          border-color: rgba(201,168,76,0.4); color: rgba(255,255,255,0.85); background: rgba(201,168,76,0.1);
        }
        .nav-drawer-lang-btn.active {
          background: var(--nav-gold); border-color: var(--nav-gold); color: var(--nav-deep);
        }
          ...
          <div className="nav-logo" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })}>
            {siteSettings?.logo_url ? (
              ...
              <svg className="nav-logo-svg" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M24 2L43 13V35L24 46L5 35V13L24 2Z" fill="rgba(201,168,76,0.08)" stroke="rgba(201,168,76,0.35)" strokeWidth="1"/>
                <path d="M24 7L39 15.5V33L24 41.5L9 33V15.5L24 7Z" fill="none" stroke="rgba(201,168,76,0.12)" strokeWidth="0.5"/>
                <path d="M24 8C19 15 13 19 13 26a11 11 0 0022 0c0-7-6-11-11-18z" fill="url(#nav-grad)"/>
                <line x1="24" y1="37" x2="24" y2="28" stroke="rgba(10,31,8,0.4)" strokeWidth="1.2" strokeLinecap="round"/>
                <path d="M24 28 Q18 24 17 19 Q21 21 24 28Z" fill="rgba(255,255,255,0.18)"/>
                <path d="M24 28 Q30 24 31 19 Q27 21 24 28Z" fill="rgba(255,255,255,0.1)"/>
                <circle cx="24" cy="10" r="1.5" fill="#c9a84c" opacity="0.7"/>
                <defs>
                  <linearGradient id="nav-grad" x1="24" y1="8" x2="24" y2="37" gradientUnits="userSpaceOnUse">
                    <stop offset="0%" stopColor="#80C756"/>
                    <stop offset="60%" stopColor="#4a9e2a"/>
                    <stop offset="100%" stopColor="#2d6e18"/>
                  </linearGradient>
                </defs>
              </svg>
            )}
